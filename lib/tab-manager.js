...
export default class TabManager {
  ...
  updateEditors = () => {
    ...
    this.branchEditors = JSON.parse(raw) || []
  }
  ...
  updateLayout = () => {
    ...
    window.localStorage.setItem(`git-tabs:new:${this.md5}:${this.currentBranchName}`, layout.serialize())
  }
  ...
  updateNewLayout = () => {
    ...
    const raw = window.localStorage.getItem(`git-tabs:new:${this.md5}:${this.currentBranchName}`)
    const branchEditors = JSON.parse(raw) || []
    console.log({branchEditors})

    // Examine how many text editor panes are open and persist to instance variable called editorPanes
    // then close any unneeded editors in each pane and open any editors that aren't open yet
  }
